...
<h2><i class="fa fa-home"></i>Data Sub Kategori <span></span></h2>
...
<li class="nav-border nav-border-top-danger"><a href="#detail" aria-controls="detail" role="tab" data-toggle="tab">Detail Data Sub Kategori</a></li>
<li class="active nav-border nav-border-top-danger"><a href="#daftar" aria-controls="daftar" role="tab" data-toggle="tab">Daftar Data Sub Kategori</a></li>
</ul>
</div>
...
<p>Yakin hapus Data Data Sub Kategori dengan nomor <strong><span id="hps_id"></span></strong>?</p>
<p class="text-right">
...
function show_detail(subktg_id = "", tab = true) {
  if (tab) {
    ganti_tab("detail");
  }
  ...
  $.ajax({
    url: site+"admin/subkategori/read",
    type: "post",
    data: {'subktg_id':subktg_id}
    })
    .done(function (data){
      $("#detail").html(data);
      })
      ...
      function baru(id = "") {
        ...
        if (id!="") {id = "/"+id}
        $.ajax({
          url: site+"admin/subkategori/create"+id,
          type: "post"
          })
          .done(function (data){
            $("#detail").html(data);
            ...
            function edit() {
              subktg_id = $("#detail input#subktg_id").val();
              if (subktg_id.trim()=="") {
                showMsg("Peringatan","Silahkan pilih data dahulu");
                  ...
                  $('#btn_batal').removeAttr("disabled");
                  $.ajax({
                    url: site+"admin/subkategori/update",
                    type: "post",
                    data: {'subktg_id':subktg_id}
                    })
                    .done(function (data){
                      ...
                      function hapus(){
                        ...
                        if (subktg_id.trim()==""){
                          showMsg("Peringatan","Silahkan pilih data dahulu");
                            ...
                            $('#modalHapus #hps_id').text(subktg_id);
                            $('#modalHapus #btnhapus').val(subktg_id);
                            $('#modalHapus').modal('show');
                          }
                        }

                        function hapus_ya(subktg_id) {
                          $.ajax({
                            url: site+"admin/subkategori/delete",
                            type: "post",
                            data: {'subktg_id':subktg_id}
                            })
                            .done(function (data){
                              ...
                              if (data.trim()=="OK") {
                                showMsg('Informasi','Data Subkategori <strong>'+subktg_id+'</strong> berhasil dihapus');
                                show_detail();
                                ...
                                function simpan() {
                                  if(valid('#detail')){
                                    ...
                                    $.ajax({
                                      url: site+"admin/subkategori/create_action",
                                      type: "post",
                                      data : post
                                      })
                                      .done(function (data){
                                        console.log(data);
                                        if (data.trim()=="simpan") {
                                          showMsg('Informasi','Data Data Sub Kategori <strong>'+post.subktg_id+'</strong> berhasil disimpan');
                                          show_detail(post.subktg_id);
                                          } else if(data.trim()=='edit') {
                                            showMsg('Informasi','Data Data Sub Kategori <strong>'+post.subktg_id+'</strong> berhasil diubah');
                                            show_detail(post.subktg_id);
                                            } else {
                                              showMsg("Peringatan",data);
                                            }
                                            history.pushState(null, null, '<?php echo base_url() ?>subkategori');
                                            })
                                            .fail(function (jqXHR, textStatus, errorThrown){
                                              console.error("The following error occurred: "+textStatus, errorThrown);
                                              console.log(jqXHR.responseText);
